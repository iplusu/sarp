...
//  ReadyMsg *readyMsgIn, *readyMsgOut, *readyAckIn;
  ...
  uint16_t numOneHopNeighbors, numOtherNeighbors, numChildren, numNeighbors, loopCounterN = 0;
  ...
  uint32_t reportQ[CIRCULAR_BUFFER_SIZE], transactionTrustToProcessQ[CIRCULAR_BUFFER_SIZE], currentTID;
  ...
  uint16_t transactionFound, sending = 0, readTransactionTrustHere = 0, writeTransactionTrustHere = 0;
  ...
  command result_t StdControl.init()
  {
    ...
    myStaticParent.parent_or_oneHop_id = NO_PARENT;
    numChildren = 0;
  ...
  task void processInMsg()
  {
    ...
    if (1) // for the time being there is no difference between root and other nodes (myAddr != 0)
    {
      ...
      else if (type == START_MSG_TYPE)
      {
        ...
        if (OkToStart)
        {
          call Messaging.putStartMsgInOutQ(outQ, &writeOutQHere, CIRCULAR_BUFFER_SIZE, &numMsgOutQ);
          post processOutMsg();
          post startApp();
          OkToStart = FALSE;
          ...
          if ( (call Attack.sfDropThis(myAddr, dataMsgIn->src_node_id, &selectiveCounter)) == 0 )
          {
            atomic
            {
              ...
              if (transactionFound == 0)
                call TransactionTable.initializeTransaction(transactionRecords, dataMsgIn->transaction_id, dataMsgIn->orig_node_id, dataMsgIn->src_node_id, myOneHopNeighbors, numOneHopNeighbors, myLevel, 0);

              /* If we are not root, forward the DATA packet to our parent. */
              if (myAddr != 0)
              {
                call Messaging.putDataMessageInOutQ(outQ, &writeOutQHere, CIRCULAR_BUFFER_SIZE, &numMsgOutQ, call TransactionTable.getTransactionParent(transactionRecords, dataMsgIn->transaction_id), myAddr, dataMsgIn->transaction_id, dataMsgIn->orig_node_id, dataMsgIn->data);
                post processOutMsg();
              }
  ...
  event TOS_MsgPtr Receive.receive(TOS_MsgPtr n)
  {
    ...
    //  dbg(DBG_USR1, "%d heard a START_MSG\r\n", myAddr);
    ...
    if ( numDataMessagesInTrial <= DATA_MESSAGES_PER_TRIAL )
    {
      if ( (myLevel != NO_LEVEL) && (myAddr != 0) )
      {
        ...
        atomic
        {
          ...
          if (transactionFound == 0)
          {
            call TransactionTable.initializeTransaction(transactionRecords, TRANSACTION_ID, myAddr, NO_PARENT, myOneHopNeighbors, numOneHopNeighbors, myLevel, 1);
            numDataMessagesInTrial = 1;
          }

          /* Put the message on the outQ */
          call Messaging.putDataMessageInOutQ(outQ, &writeOutQHere, CIRCULAR_BUFFER_SIZE, &numMsgOutQ, call TransactionTable.getTransactionParent(transactionRecords, TRANSACTION_ID), myAddr, TRANSACTION_ID, myAddr, SAMPLE_DATA);
        } // end atomic
  ...
  event result_t Timer8.fired()
  {
    ...
    if ( (numTransactionsToCheck > 0) && (call TransactionTable.getFromTransactionCheckQ(transactionsToCheckQ, &checkThisTransaction, &oneTimesTID, &twoTimesTID, &readTransactionCheckHere, CIRCULAR_BUFFER_SIZE, &numTransactionsToCheck) == 1) )
    {
      if (checkThisTransaction->processed == 0)
      {
        ...
        {
          if (checkThisTransaction->processed == 0)
            call TransactionTable.processThisTransaction(checkThisTransaction, neighbors, numNeighbors);
          if (call TrafficGenerator.isTrafficGenerator(myAddr) == 1)
        ...
        else if (checkThisTransaction->oneHopReported == 0)
        {
          ...
          else
          {
            ...
            #ifdef DEBUG
            dbg(DBG_USR1, "%d reports ONE hop %d DID NOT CHECK IN %d times for transaction %x\r\n", myAddr, checkThisTransaction->packetsForwardedTo, oneTimesTID + 1, checkThisTransaction->transaction_id);
            #endif
