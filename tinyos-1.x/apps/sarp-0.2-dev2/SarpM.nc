...
implementation {
  ...
  uint32_t DFThreshold = 2880; // number of 30-seconds
      ...
      else if (type == DATA_MSG_TYPE)
      {
        ...
        if (dataMsgIn->dst_node_id == myAddr)
        {
          ...
          if ( (call Attack.sfDropThis(myAddr, dataMsgIn->src_node_id, &selectiveCounter)) == 0 )
          {
            ...
            if  ( ((call TransactionTable.isALoop(transactionRecords, dataMsgIn->transaction_id, dataMsgIn->src_node_id) == 1) && (call Attack.isLoopCreator(myAddr) != 1 ) )  || ( (lastToFindLoop == myAddr) && (cantFixIt == 0) ) || (cantFixIt == 1) )
            {
              ...
              if (myAddr != 0)
              {
                ...
                call Messaging.putDataMessageInOutQ(outQ, &writeOutQHere, CIRCULAR_BUFFER_SIZE, &numMsgOutQ, newTransactionParent, myAddr, dataMsgIn->transaction_id, dataMsgIn->orig_node_id, dataMsgIn->data, lastToFindLoop, cantFixIt, dontSendTo);
    ...
    if ( numDataMessagesInTrial <= DATA_MESSAGES_PER_TRIAL )
    {
      if ( (myLevel != NO_LEVEL) && (myAddr != 0) )
      {
        ...
        atomic
        {
          ...
          call Messaging.putDataMessageInOutQ(outQ, &writeOutQHere, CIRCULAR_BUFFER_SIZE, &numMsgOutQ, call TransactionTable.getTransactionParent(transactionRecords, TRANSACTION_ID), myAddr, TRANSACTION_ID, myAddr, SAMPLE_DATA, lastToFindLoop, cantFixIt, dontSendTo);
        } // end atomic
