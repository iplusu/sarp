...
implementation {
   ...
   */
  command void TransactionTable.adjustThisNeighborsOverallTrust(Neighbor_Rec_withTrust_t neighbors[], uint16_t node_id, uint16_t numNeighbors)
  {
    for (tnLoopCounter = 0; tnLoopCounter < numNeighbors; tnLoopCounter ++)
    {
      if (neighbors[tnLoopCounter].node_id == node_id)
      {
        neighbors[tnLoopCounter].trust.overallTrust = call Trust.calculateOverallTrust(neighbors[tnLoopCounter].trust.trustFP, neighbors[tnLoopCounter].trust.trustRH, neighbors[tnLoopCounter].trust.trustAV);
        break;
      }
    }
  }
  ...
  command void TransactionTable.processThisTransaction(Transaction_Record_Ptr thisTransaction, Neighbor_Rec_withTrust_t neighbors[], uint16_t numNeighbors)
  {
    ...
    if (thisTransaction->oneHopReported == 1)
    {
      if (thisTransaction->twoHopReported == 1)
      {
        ...
        if (thisTransaction->packetsForwardedTo != 0)
          call TransactionTable.adjustThisNeighborsAVTrust(neighbors, thisTransaction->packetsForwardedTo, numNeighbors, 0, GOOD_AVAILABILITY_BEHAVIOR_INCREMENT);
        if (thisTransaction->oneHopReportedPacketsForwardedTo != 0)
          call TransactionTable.adjustThisNeighborsAVTrust(neighbors, thisTransaction->oneHopReportedPacketsForwardedTo, numNeighbors, 0, GOOD_AVAILABILITY_BEHAVIOR_INCREMENT);
      }
      else // twoHopReported == 255
      {
        ...
        if (thisTransaction->packetsForwardedTo != 0)
          call TransactionTable.adjustThisNeighborsAVTrust(neighbors, thisTransaction->packetsForwardedTo, numNeighbors, 0, GOOD_AVAILABILITY_BEHAVIOR_INCREMENT);
        if (thisTransaction->oneHopReportedPacketsForwardedTo != 0)
          call TransactionTable.adjustThisNeighborsAVTrust(neighbors, thisTransaction->oneHopReportedPacketsForwardedTo, numNeighbors, 0, BAD_AVAILABILITY_BEHAVIOR_INCREMENT);
      }
    }
    else // oneHopReported == 255
    {
      if (thisTransaction->twoHopReported == 1)
      {
        ...
        if (thisTransaction->packetsForwardedTo != 0)
          call TransactionTable.adjustThisNeighborsAVTrust(neighbors, thisTransaction->packetsForwardedTo, numNeighbors, 0, BAD_AVAILABILITY_BEHAVIOR_INCREMENT);
        if (thisTransaction->oneHopReportedPacketsForwardedTo != 0)
          call TransactionTable.adjustThisNeighborsAVTrust(neighbors, thisTransaction->oneHopReportedPacketsForwardedTo, numNeighbors, 0, GOOD_AVAILABILITY_BEHAVIOR_INCREMENT);
      }
      else // twoHopReported == 255
    ...
    /* Now tha the parts are processed, proicess the overallTrust */
    call TransactionTable.adjustThisNeighborsOverallTrust(neighbors, thisTransaction->packetsForwardedTo, numNeighbors);
   ...
   */
  command uint16_t TransactionTable.isALoop(Transaction_Record_t transactions[], uint32_t transaction_id, uint16_t senderNow)
    {
     for (tLoopCounter = 0; tLoopCounter < MAX_TRANSACTIONS; tLoopCounter ++)
     {
       if (transactions[tLoopCounter].transaction_id == transaction_id)
       {
         if(transactions[tLoopCounter].receivedFrom != senderNow)
          return 1;
         else
           return 0;
       }
