...
implementation {
   ...
   */
  command void TransactionTable.adjustThisNeighborsOverallTrust(Neighbor_Rec_withTrust_t neighbors[], uint16_t node_id, uint16_t numNeighbors, uint16_t whoIAm, uint32_t transactionNumber)
  {
    for (tnLoopCounter = 0; tnLoopCounter < numNeighbors; tnLoopCounter ++)
    {
      if (neighbors[tnLoopCounter].node_id == node_id)
  ...
  command void TransactionTable.processThisTransaction(Transaction_Record_Ptr thisTransaction, Neighbor_Rec_withTrust_t neighbors[], uint16_t numNeighbors, uint16_t whoIAm)
  {
    ...
    if (thisTransaction->isALoop == 1)
      call TransactionTable.adjustThisNeighborsLPTrust(neighbors, thisTransaction->packetsForwardedTo, numNeighbors, 0, 1);
    else
      call TransactionTable.adjustThisNeighborsLPTrust(neighbors, thisTransaction->packetsForwardedTo, numNeighbors, 1, 0);
    ...
    call TransactionTable.adjustThisNeighborsOverallTrust(neighbors, thisTransaction->packetsForwardedTo, numNeighbors, whoIAm, thisTransaction->transaction_id);
