...
interface TransactionTable
{
	 ...
	 */
	#ifdef ONOFFATTACK
	command void processThisTransaction(Transaction_Record_Ptr thisTransaction, Neighbor_Rec_withTrust_t neighbors[], uint16_t numNeighbors, uint16_t whoIAm, uint8_t isGood, uint32_t timeCounter);
	#else
	 ...
	 */
	#ifdef ONOFFATTACK
	command void adjustThisNeighborsOverallTrust(Neighbor_Rec_withTrust_t neighbors[], uint16_t node_id, uint16_t numNeighbors, uint16_t whoIAm, uint32_t transactionNumber, uint32_t GFBinc, uint32_t BFBinc, uint32_t GLBinc, uint32_t BLBinc, uint8_t isGood, uint32_t t);
	#else
