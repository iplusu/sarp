	...
	#endif
	{
		...
		for (tnLoopCounter = 0; tnLoopCounter < numNeighbors; tnLoopCounter++)
		{
			if (neighbors[tnLoopCounter].node_id == node_id)
			{
				...
				#endif
				dbg(DBG_USR3, "transaction:%d node:%d fp:%f pr:%f ov:%f\r\n", transactionNumber, neighbors[tnLoopCounter].node_id, neighbors[tnLoopCounter].trust.trustFP, neighbors[tnLoopCounter].trust.trustPR, neighbors[tnLoopCounter].trust.overallTrust);
				#endif
			}
...
//			if (queue[tIndex].transactionPtr == ptr)
	...
	command void TransactionTable.doDynamicForgetting(Neighbor_Rec_withTrust_t neighbors[], uint16_t numNeighbors, uint16_t whoIAm, uint32_t timeCounter)
	{
		for (tnLoopCounter = 0; tnLoopCounter < numNeighbors; tnLoopCounter++)
		{
			...
			neighbors[tnLoopCounter].trust.trustFP = neighbors[tnLoopCounter].trust.trustFP * call Trust.calculateForgettingFactor(neighbors[tnLoopCounter].trust.GFB, neighbors[tnLoopCounter].trust.BFB);
			if(neighbors[tnLoopCounter].trust.trustFP > 1.0)		neighbors[tnLoopCounter].trust.trustFP = 1.0;
			else if (neighbors[tnLoopCounter].trust.trustFP < 0.1)	neighbors[tnLoopCounter].trust.trustFP = 0.1;
			
			neighbors[tnLoopCounter].trust.overallTrust = call Trust.calculateOverallTrust(neighbors[tnLoopCounter].trust.trustFP, neighbors[tnLoopCounter].trust.trustLP);
			
			if(neighbors[tnLoopCounter].trust.overallTrust > 1)	neighbors[tnLoopCounter].trust.overallTrust = 1.0;
			else if(neighbors[tnLoopCounter].trust.overallTrust < 0.1)	neighbors[tnLoopCounter].trust.overallTrust = 0.1;
