...
//#undef SWINDOW_TIME
//#define SWINDOW_TIME			(1000 * 60 * 60)
//#endif
...
module SarpM {
	...
	uses {
    	...
    	interface SendMsg as SendNeighborsTrust;
    	interface SendMsg as SendTransaction;
...
//    interface Timer as Timer5;          /* Sends an end-of-test Ready Message out of the UART   */
		...
		/* JRT */
		interface Timer as Timer11;
		interface Timer as Timer12;
...
#ifdef PREDICTABILITY
#ifdef __REAL__MOTE__
	uint8_t prevNeighborsAllocBusy = 1;
	uint8_t prevNeighborsAllocated = 0;
	Handle memHandle;
#endif
#endif
...
//  ReadyMsg *readyMsgIn, *readyMsgOut, *readyAckIn;
	...
	uint16_t oneTimesTID, twoTimesTID = 0;
...
//	unsigned long int timeCounter = 1;
//	uint32_t SWINDOW_TIME = (uint32_t)1000*60*60;
	
	task void startApp()
	{
		dbg(DBG_USR1, "%d started its App.\r\n", TOS_LOCAL_ADDRESS);
		this_dmesg.type = AM_CONTROLMSG;
		call Leds.redOff();
		call Leds.greenOff();
		call Leds.yellowOff();
    	...
    	/* JRT */
    	if (call Attack.isSfAttacker(myAddr))
    	{
    		call Leds.redOn();
    	}
    			
//		call Timer5.start(TIMER_REPEAT, SWINDOW_TIME);
...
#ifdef __REAL__MOTE__
		call CC2420Control.SetRFPower(30);
	...
	/* JRT */
	event result_t SendUart.sendDone(TOS_MsgPtr masg, result_t success)
	{
		sending = 0;
	  	return SUCCESS;
	}
	event result_t SendNeighborsTrust.sendDone(TOS_MsgPtr masg, result_t success)
	{
	 	sending = 0;
		return SUCCESS;
	}
	event result_t SendTransaction.sendDone(TOS_MsgPtr masg, result_t success)
	{
		sending = 0;
	  	return SUCCESS;
	}  
	...
	int randomOnOffAttack(int g){
		int randNum = 0;
		#ifdef __REAL__MOTE__
		uint32_t cTime = call LocalTime.read();
		#else
	...
	task void processInMsg()
	{
		...
		if (1) // for the time being there is no difference between root and other nodes (myAddr != 0)
    	{
			...
			{
				/* JRT */
				if (call Messaging.isMsgForMe(myAddr, disc_msg_in->node_id)) {
					#ifdef DEBUG
					...
					#endif
					call NeighborTable.processNeighborFromDiscMsg(neighbors, myOneHopNeighbors, &numNeighbors, &numOneHopNeighbors, disc_msg_in->node_id, disc_msg_in->level, disc_msg_in->parent_id, &myOne, cTime);
					for (loopCounter = 0; loopCounter < disc_msg_in->num_neighbors; loopCounter++)
					{
						if (disc_msg_in->neighbors[loopCounter].node_id != myAddr)
							call NeighborTable.processNeighborFromDiscMsg(neighbors, myOneHopNeighbors, &numNeighbors, &numOneHopNeighbors, disc_msg_in->neighbors[loopCounter].node_id, disc_msg_in->neighbors[loopCounter].level, disc_msg_in->node_id, &myZero, cTime);
					}
			...
			else if (type == AM_CONTROLMSG)
			{
				/* TO DO : reset mote here if running */
				controlMsgIn = (ControlMsg *) inQ[readInQHere].data;
									
				/* JRT */
				if (call Messaging.isMsgForMe(myAddr, controlMsgIn->data[0])) {
					#ifdef DEBUG
			...
			else if (type == DATA_MSG_TYPE)
			{
				...
				/* JRT */		
				if (call Messaging.isMsgForMe(myAddr, dataMsgIn->src_node_id)) {
			...
			else if (type == REPORT_REQUEST_MSG_TYPE) // set isValid = 0
			{
				...
				if (call Messaging.isMsgForMe(myAddr, reportReqMsgIn->requestor)) {
			...
			else if (type == REPORT_RESPONSE_NO_FORWARD_MSG_TYPE)
			{
				...
				/* JRT */		
				if (call Messaging.isMsgForMe(myAddr, reportRespNoForwardMsgIn->responder)) {       				
					#ifdef DEBUG
			...
			else if (type == REPORT_RESPONSE_FORWARD_MSG_TYPE)
			{
				atomic
				{
					...
					/* JRT */		
		  			if (call Messaging.isMsgForMe(myAddr, reportForwardMsgIn->responder)) {    					
		  				#ifdef DEBUG
			...
			else if (type == END_TO_END_REPORT_RESPONSE_MSG_TYPE) // set isValid = 0
			{
				...
				/* JRT */		
				if (call Messaging.isMsgForMe(myAddr, endToEndRepRespMsgIn->responder)) {     				
			...
			else if (type == SPECIFIC_REPORT_REQUEST_MSG_TYPE)
			{
				...
				/* JRT */		
		  		if (call Messaging.isMsgForMe(myAddr, specificReportReqMsgIn->requestor)) {
					#ifdef SARP04_DBG
	...
	//		 				dbg(DBG_USR3, "%d received a SPECIFIC_REPORT_REQUEST_MSG for ME.\r\n", myAddr);
							call Messaging.putSpecificReportResponseMsgInOutQ(outQ, &writeOutQHere, CIRCULAR_BUFFER_SIZE, &numMsgOutQ, myAddr, specificReportReqMsgIn->originator, specificReportReqMsgIn->transaction_id, thisTransaction->packetsReceived, thisTransaction->receivedFrom, thisTransaction->packetsForwarded, thisTransaction->packetsForwardedTo, thisTransaction->receivedFrom, myAddr);
							dbg(DBG_USR1, "%d\r\n",__LINE__);
							post processOutMsg();
							dbg(DBG_USR1, "%d\r\n",__LINE__);
						}
	...
	//						dbg(DBG_USR3, "%d received a SPECIFIC_REPORT_REQUEST_MSG for %d.\r\n", myAddr, specificReportReqMsgIn->destination);
		    				#endif
							call Messaging.putSpecificReportRequestMsgInOutQ(outQ, &writeOutQHere, CIRCULAR_BUFFER_SIZE, &numMsgOutQ, thisTransaction->packetsForwardedTo, thisTransaction->packetsForwardedTo, myAddr, specificReportReqMsgIn->originator, specificReportReqMsgIn->originatorPacketCount, specificReportReqMsgIn->transaction_id);
							#ifdef SARP04_DBG
	...
	//						dbg(DBG_USR3, "%d does not set OutMsg for SPECIFIC_REPORT_REQUEST_MSG.\r\n", myAddr);
			...
			else if (type == SPECIFIC_REPORT_RESPONSE_MSG_TYPE)
			{
				...
				/* JRT */		
		  		if (call Messaging.isMsgForMe(myAddr, specificReportResponseMsgIn->forwarder)) {
					atomic
					{
						for (transactionCounter = 0; transactionCounter < MAX_TRANSACTIONS; transactionCounter++)
						{
							if ( transactionRecords[transactionCounter].transaction_id == specificReportResponseMsgIn->transaction_id)
							{
								...
								{
									call Messaging.putSpecificReportResponseMsgInOutQ(outQ, &writeOutQHere, CIRCULAR_BUFFER_SIZE, &numMsgOutQ, specificReportResponseMsgIn->responder, specificReportResponseMsgIn->originator, specificReportResponseMsgIn->transaction_id, specificReportResponseMsgIn->packetsReceived, specificReportResponseMsgIn->packetsReceivedFrom, specificReportResponseMsgIn->packetsForwarded, specificReportResponseMsgIn->packetsForwardedTo, specificReportResponseMsgIn->originator, myAddr);
									post processOutMsg();
									#ifdef DEBUG
...
//		dbg(DBG_USR3, "addr: %d type: %d group:%d length:%d Data Length: %d\r\n", n->addr, n->type, n->group, n->length, TOS_DATA_LENGTH);
	...
	task void sendDataMsg()
	{
		...
		else
		{
		    /* JRT */
      		call Leds.greenOff();
			call Timer11.start(TIMER_ONE_SHOT, 5000);
		}
	}
	...
	{
    	call Messaging.putTrustDataToUartMsgInOutQ(outQ, &writeOutQHere, CIRCULAR_BUFFER_SIZE, &numMsgOutQ, neighbors, numNeighbors);
        post processOutMsg();	
	}
...
//	 	dbg(DBG_USR3, "Timer6!!!\r\n");
...
#ifdef PREDICTABILITY
#ifdef __REAL__MOTE__
		if (!prevNeighborsAllocated)
		{
			call MemAlloc.allocate(&memHandle, sizeof(Trust_Rec_t) * numNeighbors);
			prevNeighborsAllocated = 1;
		}
		if (prevNeighborsAllocBusy)
		{
			call Timer8.start(TIMER_ONE_SHOT, 10);
			return SUCCESS;
		}
#endif
#endif
			...
			if (checkThisTransaction->processed == 0)
			{
				...
				if ( (checkThisTransaction->oneHopReported > 0) && ( (checkThisTransaction->twoHopReported > 0) || (checkThisTransaction->packetsForwardedTo == 0)) )
				{
					if (checkThisTransaction->processed == 0) {
...
#ifdef ONOFFATTACK
						if(nGoodBehaviors >= 0)
							call TransactionTable.processThisTransaction(checkThisTransaction, neighbors, numNeighbors, myAddr, TRUE, cTime, &memHandle);
						else
							call TransactionTable.processThisTransaction(checkThisTransaction, neighbors, numNeighbors, myAddr, FALSE, cTime, &memHandle);
							
						call MemAlloc.free(memHandle);
						prevNeighborsAllocBusy = 1;
						prevNeighborsAllocated = 0;
#else
					    ...
					    /* JRT */
					    if (call TrafficGenerator.isTrafficGenerator(myAddr) == 1) {
            				call Messaging.putTransactionDataToUartMsgInOutQ(outQ, &writeOutQHere, CIRCULAR_BUFFER_SIZE, &numMsgOutQ, checkThisTransaction);          	
            				post processOutMsg();
	           			}            			
					}
					...
					{
						/* JRT */
						call Timer12.start(TIMER_ONE_SHOT, 5000); 
	...
	event result_t Timer10.fired()
	{
		...
		if (DFCounter >= DFThreshold)
		{
			...
			call TransactionTable.doDynamicForgetting(neighbors, numNeighbors, myAddr, cTime);
			#endif
		}
		return SUCCESS;
	}
