...
//#if (0)
	...
	/* JRT */
  	SarpM.SendUart -> UComm.SendMsg[AM_CONTROLMSG];
  	SarpM.SendNeighborsTrust -> UComm.SendMsg[AM_NEIGHBORSTRUSTMSG];
  	SarpM.SendTransaction -> UComm.SendMsg[AM_TRANSACTIONDATAMSG];

	SarpM.SendDisc->Comm.SendMsg[DISCOVERY_MSG_TYPE];
	//SarpM.SendUDisc->UComm.SendMsg[DISCOVERY_MSG_TYPE];
	SarpM.SendStart->Comm.SendMsg[AM_CONTROLMSG];
	...
	SarpM.Receive->Comm.ReceiveMsg[AM_CONTROLMSG];
...
//  SarpM.Timer5 -> TimerC.Timer[unique ("Timer")];
	...
	/* JRT */
	SarpM.Timer11->TimerC.Timer[unique("Timer")];
	SarpM.Timer12->TimerC.Timer[unique("Timer")];
