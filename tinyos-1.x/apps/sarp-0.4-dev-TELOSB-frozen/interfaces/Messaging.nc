...
interface Messaging
{
	 ...
	 *    C
	 */
	command void putDataMessageInOutQ(TOS_Msg outQ[], uint16_t *writeHere, uint16_t maxQSize, uint16_t *numMsgOutQ, uint16_t dst_node_id, uint16_t src_node_id, uint32_t transaction_id, uint16_t orig_node_id, uint16_t data, uint16_t lastToFindLoop, uint16_t canFixIt, uint16_t dontSendTo, uint16_t sequenceNumberIn);
	...
	command void putNeighborDiscoveryControlMsgInOutQ(TOS_Msg outQ[], uint16_t *writeHere, uint16_t maxQSize, uint16_t *numMsgOutQ, uint16_t whoIAm, uint16_t whoIHeard, uint16_t parent);
 	...
 	/* JRT */ 
	command void putTrustDataToUartMsgInOutQ(TOS_Msg outQ[], uint16_t *writeHere, uint16_t maxQSize, uint16_t *numMsgOutQ, Neighbor_Rec_withTrust_t neighbors[], uint16_t numNeighbors);
	command void putDiscMsgInUartQ(TOS_Msg outQ[], uint16_t *writeHere, uint16_t maxQSize, uint16_t *numMsgOutQ, uint16_t sender);
	command void putTransactionDataToUartMsgInOutQ(TOS_Msg outQ[], uint16_t *writeHere, uint16_t maxQSize, uint16_t *numMsgOutQ, Transaction_Record_Ptr thisTransaction);
	command int isMsgForMe(uint16_t myAddr, uint16_t node_id);
}
