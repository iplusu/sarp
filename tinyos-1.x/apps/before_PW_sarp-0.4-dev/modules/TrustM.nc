...
//      return 1.0;
	...
	command float Trust.calculatePR(PB_t PB_1)
	{
		...
		for(cnt = 0; cnt < SWINDOW_SIZE; cnt++) {
			if(((tos_state.tos_time/4000) - PB_1.SWindow[cnt].time > SWINDOW_TIME_OUT) && (PB_1.SWindow[cnt].time != 0)) {
#ifdef SARP04
		...
		p = (GPB + 1.0) / (GPB + BPB + 2.0);
		n = (BPB + 1.0) / (GPB + BPB + 2.0);
		return (1 - p - n);
	}
...
#ifdef TR
		dbg(DBG_USR3, "GB: %d BB: %d trustPR:%f TMP:%f TRUST: %f\n", GB_1, BB_1, trustPR, tmp, 1.0 + tmp);
#endif
		return 1.0 + tmp;
	}
}
