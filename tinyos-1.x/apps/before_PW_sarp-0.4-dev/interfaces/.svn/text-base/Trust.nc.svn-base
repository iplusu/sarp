...
interface Trust
{
  ...
  command float calculatePR(uint32_t GPB_l, uint32_t BPB_l);
  command float calculateOverallTrust(float trustFP, float trustRH, float trustAV, float trustLP, float trustPR);
