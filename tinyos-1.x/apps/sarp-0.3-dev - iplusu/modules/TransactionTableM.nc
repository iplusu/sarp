...
implementation {
  ...
  command void TransactionTable.processThisTransaction(Transaction_Record_Ptr thisTransaction, Neighbor_Rec_withTrust_t neighbors[], uint16_t numNeighbors, uint16_t whoIAm)
  {
    ...
    if (thisTransaction->oneHopReported == 1)
    {
      if (thisTransaction->twoHopReported == 1)
      {
    	...
    	one_bfb_inc = (thisTransaction->packetsForwarded - thisTransaction->twoHopReportedPacketsReceived) * BAD_FORWARDING_BEHAVIOR_INCREMENT;//thisTransaction->oneHopReportedPacketsReceived;
		#ifdef IPLUSU1
		dbg(DBG_USR3, "%d: one_gfb_inc:%d one_bfb_inc:%d\n", __LINE__, one_gfb_inc, one_bfb_inc);
		dbg(DBG_USR3, "%d: packetsForwarded:%d twoHopReportedPacketsReceived:%d result:%d\n", __LINE__, thisTransaction->packetsForwarded, thisTransaction->twoHopReportedPacketsReceived, thisTransaction->packetsForwarded - thisTransaction->twoHopReportedPacketsReceived);
		#endif
      ...
      else // twoHopReported == 255
      {
    	...
    	one_bfb_inc = (thisTransaction->packetsForwarded - thisTransaction->twoHopReportedPacketsReceived) * 50;//thisTransaction->oneHopReportedPacketsReceived;
    ...
    else // oneHopReported == 255
    {
      ...
      else // twoHopReported == 255
      {
        ...
        one_bab_inc =BAD_AVAILABILITY_BEHAVIOR_INCREMENT ;
		#ifdef IPLUSU1
		dbg(DBG_USR3, "%d: one_gab_inc:%d one_bab_inc:%d", __LINE__, one_gab_inc, one_bab_inc);
		#endif
      }
    }

	#ifdef IPLUSU1
    dbg(DBG_USR3, "%d: node_id:%d numNeighbors:%d whoIAm:%d GFBinc:%d BFBinc:%d GRBinc:%d BRBinc:%d GABinc:%d BABinc:%d GLBinc:%d BLBinc:%d\n", __LINE__, thisTransaction->packetsForwardedTo, numNeighbors, whoIAm, one_gfb_inc, one_bfb_inc, one_grb_inc, one_brb_inc, one_gab_inc, one_bab_inc, one_glb_inc, one_blb_inc);
    dbg(DBG_USR3, "%d: node_id:%d numNeighbors:%d whoIAm:%d GFBinc:%d BFBinc:%d GRBinc:%d BRBinc:%d GABinc:%d BABinc:%d GLBinc:%d BLBinc:%d\n", __LINE__, thisTransaction->packetsForwardedTo, numNeighbors, whoIAm, two_gfb_inc, two_bfb_inc, two_grb_inc, two_brb_inc, two_gab_inc, two_bab_inc, two_glb_inc, two_blb_inc);
	#endif
  ...
  command void TransactionTable.doDynamicForgetting(Neighbor_Rec_withTrust_t neighbors[], uint16_t numNeighbors, uint16_t whoIAm)
  {
    for (tnLoopCounter = 0; tnLoopCounter < numNeighbors; tnLoopCounter ++)
    {
	  ...
	  neighbors[tnLoopCounter].trust.trustAV = call Trust.calculateAV(neighbors[tnLoopCounter].trust.GAB, neighbors[tnLoopCounter].trust.BAB);
	  neighbors[tnLoopCounter].trust.trustLP = call Trust.calculateLP(neighbors[tnLoopCounter].trust.GLB, neighbors[tnLoopCounter].trust.BLB);
	  neighbors[tnLoopCounter].trust.trustPR = call Trust.calculatePR(neighbors[tnLoopCounter].trust.GPB, neighbors[tnLoopCounter].trust.BPB);*/
 	  #ifdef IPLUSU1
	  dbg(DBG_USR3, "[%d] FP: (%d / (%d + %d)) * %f", neighbors[tnLoopCounter].node_id, neighbors[tnLoopCounter].trust.GFB, neighbors[tnLoopCounter].trust.GFB, neighbors[tnLoopCounter].trust.BFB, (1.0 + (neighbors[tnLoopCounter].trust.trustFP * DYNAMIC_FORGETTING_WEIGHT)));
	  #endif
	  neighbors[tnLoopCounter].trust.trustFP = (call Trust.calculateFP(neighbors[tnLoopCounter].trust.GFB, neighbors[tnLoopCounter].trust.BFB)) * (1.0 + (neighbors[tnLoopCounter].trust.trustFP * DYNAMIC_FORGETTING_WEIGHT));
	  if(neighbors[tnLoopCounter].trust.trustFP > 1)	neighbors[tnLoopCounter].trust.trustFP = 1.0;
	  else if(neighbors[tnLoopCounter].trust.trustFP == 0)	neighbors[tnLoopCounter].trust.trustFP = 0.01;
	  neighbors[tnLoopCounter].trust.GFB = neighbors[tnLoopCounter].trust.trustFP * 100;
	  neighbors[tnLoopCounter].trust.BFB = 100 - neighbors[tnLoopCounter].trust.GFB;
	  #ifdef IPLUSU1
	  dbg(DBG_USR3, " = %f\n", neighbors[tnLoopCounter].trust.trustFP);
	  /*-----------------------------------------------------------------------------------------*/
	  dbg(DBG_USR3, "[%d] RH: (%d / (%d + %d)) * %f", neighbors[tnLoopCounter].node_id, neighbors[tnLoopCounter].trust.GRB, neighbors[tnLoopCounter].trust.GRB, neighbors[tnLoopCounter].trust.BRB, (1.0 + (neighbors[tnLoopCounter].trust.trustRH * DYNAMIC_FORGETTING_WEIGHT)));
	  #endif
	  neighbors[tnLoopCounter].trust.trustRH = (call Trust.calculateRH(neighbors[tnLoopCounter].trust.GRB, neighbors[tnLoopCounter].trust.BRB)) * (1.0 + (neighbors[tnLoopCounter].trust.trustRH * DYNAMIC_FORGETTING_WEIGHT));
	  if(neighbors[tnLoopCounter].trust.trustRH > 1)	neighbors[tnLoopCounter].trust.trustRH = 1.0;
	  else if(neighbors[tnLoopCounter].trust.trustRH == 0)	neighbors[tnLoopCounter].trust.trustRH = 0.01;
	  neighbors[tnLoopCounter].trust.GRB = neighbors[tnLoopCounter].trust.trustRH * 100;
	  neighbors[tnLoopCounter].trust.BRB = 100 - neighbors[tnLoopCounter].trust.GRB;
      #ifdef IPLUSU1
	  dbg(DBG_USR3, " = %f\n", neighbors[tnLoopCounter].trust.trustRH);
	  /*-----------------------------------------------------------------------------------------*/
	  dbg(DBG_USR3, "[%d] AV: (%d / (%d + %d)) * %f", neighbors[tnLoopCounter].node_id, neighbors[tnLoopCounter].trust.GAB, neighbors[tnLoopCounter].trust.GAB, neighbors[tnLoopCounter].trust.BAB, (1.0 + (neighbors[tnLoopCounter].trust.trustAV * DYNAMIC_FORGETTING_WEIGHT)));
      #endif
	  neighbors[tnLoopCounter].trust.trustAV = (call Trust.calculateAV(neighbors[tnLoopCounter].trust.GAB, neighbors[tnLoopCounter].trust.BAB)) * (1.0 + (neighbors[tnLoopCounter].trust.trustAV * DYNAMIC_FORGETTING_WEIGHT));
	  if(neighbors[tnLoopCounter].trust.trustAV > 1)	neighbors[tnLoopCounter].trust.trustAV = 1.0;
	  else if(neighbors[tnLoopCounter].trust.trustAV == 0)	neighbors[tnLoopCounter].trust.trustAV = 0.01;
	  neighbors[tnLoopCounter].trust.GAB = neighbors[tnLoopCounter].trust.trustAV * 100;
	  neighbors[tnLoopCounter].trust.BAB = 100 - neighbors[tnLoopCounter].trust.GAB;
      #ifdef IPLUSU1
	  dbg(DBG_USR3, " = %f\n", neighbors[tnLoopCounter].trust.trustAV);
	  /*-----------------------------------------------------------------------------------------*/
	  dbg(DBG_USR3, "[%d] LP: (%d / (%d + %d)) * %f", neighbors[tnLoopCounter].node_id, neighbors[tnLoopCounter].trust.GLB, neighbors[tnLoopCounter].trust.GLB, neighbors[tnLoopCounter].trust.BLB, (1.0 + (neighbors[tnLoopCounter].trust.trustLP * DYNAMIC_FORGETTING_WEIGHT)));
      #endif
	  neighbors[tnLoopCounter].trust.trustLP = (call Trust.calculateLP(neighbors[tnLoopCounter].trust.GLB, neighbors[tnLoopCounter].trust.BLB)) * (1.0 + (neighbors[tnLoopCounter].trust.trustLP * DYNAMIC_FORGETTING_WEIGHT));
	  if(neighbors[tnLoopCounter].trust.trustLP > 1)	neighbors[tnLoopCounter].trust.trustLP = 1.0;
  	  else if(neighbors[tnLoopCounter].trust.trustLP == 0)	neighbors[tnLoopCounter].trust.trustLP = 0.01;
	  neighbors[tnLoopCounter].trust.GLB = neighbors[tnLoopCounter].trust.trustLP * 100;
	  neighbors[tnLoopCounter].trust.BLB = 100 - neighbors[tnLoopCounter].trust.GLB;
      #ifdef IPLUSU1
	  dbg(DBG_USR3, " = %f\n", neighbors[tnLoopCounter].trust.trustLP);
	  /*-----------------------------------------------------------------------------------------*/
	  dbg(DBG_USR3, "[%d] PR: (%d / (%d + %d)) * %f", neighbors[tnLoopCounter].node_id, neighbors[tnLoopCounter].trust.GPB, neighbors[tnLoopCounter].trust.GPB, neighbors[tnLoopCounter].trust.BPB, (1.0 + (neighbors[tnLoopCounter].trust.trustPR * DYNAMIC_FORGETTING_WEIGHT)));
      #endif
	  neighbors[tnLoopCounter].trust.trustPR = (call Trust.calculatePR(neighbors[tnLoopCounter].trust.GPB, neighbors[tnLoopCounter].trust.BPB)) * (1.0 + (neighbors[tnLoopCounter].trust.trustPR * DYNAMIC_FORGETTING_WEIGHT));
	  if(neighbors[tnLoopCounter].trust.trustPR > 1)	neighbors[tnLoopCounter].trust.trustPR = 1.0;
	  else if(neighbors[tnLoopCounter].trust.trustPR == 0)	neighbors[tnLoopCounter].trust.trustPR = 0.01;
	  ...
	  neighbors[tnLoopCounter].trust.overallTrust = call Trust.calculateOverallTrust(neighbors[tnLoopCounter].trust.trustFP, neighbors[tnLoopCounter].trust.trustRH, neighbors[tnLoopCounter].trust.trustAV, neighbors[tnLoopCounter].trust.trustLP, neighbors[tnLoopCounter].trust.trustPR);
      #ifdef DEBUG1
      dbg(DBG_USR3, "START DYNAMIC FORGETTING\n");
      dbg(DBG_USR3, "[%d] GFB:%d BFB:%d trustFP:%f\n", neighbors[tnLoopCounter].node_id, neighbors[tnLoopCounter].trust.GFB, neighbors[tnLoopCounter].trust.BFB, neighbors[tnLoopCounter].trust.trustFP);
      dbg(DBG_USR3, "[%d] GRB:%d BRB:%d trustRH:%f\n", neighbors[tnLoopCounter].node_id, neighbors[tnLoopCounter].trust.GRB, neighbors[tnLoopCounter].trust.BRB, neighbors[tnLoopCounter].trust.trustRH);
      dbg(DBG_USR3, "[%d] GAB:%d BAB:%d trustAV:%f\n", neighbors[tnLoopCounter].node_id, neighbors[tnLoopCounter].trust.GAB, neighbors[tnLoopCounter].trust.BAB, neighbors[tnLoopCounter].trust.trustAV);
      dbg(DBG_USR3, "[%d] GLB:%d BLB:%d trustLP:%f\n", neighbors[tnLoopCounter].node_id, neighbors[tnLoopCounter].trust.GLB, neighbors[tnLoopCounter].trust.BLB, neighbors[tnLoopCounter].trust.trustLP);
      dbg(DBG_USR3, "[%d] GPB:%d BPB:%d trustPR:%f\n", neighbors[tnLoopCounter].node_id, neighbors[tnLoopCounter].trust.GPB, neighbors[tnLoopCounter].trust.BPB, neighbors[tnLoopCounter].trust.trustPR);
      dbg(DBG_USR3, "[%d] overallTrust:%f\n", neighbors[tnLoopCounter].node_id, neighbors[tnLoopCounter].trust.overallTrust);
	  dbg(DBG_USR3, "FINISH DYNAMIC FORGETTING\n");
      #endif
