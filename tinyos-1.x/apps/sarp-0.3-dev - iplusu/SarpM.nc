          ...
          for (transactionCounter = 0; transactionCounter < MAX_TRANSACTIONS; transactionCounter ++)
          {
            ...
            if ( transactionRecords[transactionCounter].transaction_id == reportRespNoForwardMsgIn->transaction_id)
            {
              ...
              else if (reportRespNoForwardMsgIn->packetsReceivedFrom == transactionRecords[transactionCounter].packetsForwardedTo)
              {
				...
				#ifdef DEBUG3
				dbg(DBG_USR1, "forwarded:%d responder:%d two hop received:%d r-received:%d\n", transactionRecords[transactionCounter].oneHopReportedPacketsForwardedTo, reportRespNoForwardMsgIn->responder, transactionRecords[transactionCounter].twoHopReportedPacketsReceived, reportRespNoForwardMsgIn->packetsReceived);
				dbg(DBG_USR3, "forwarded:%d responder:%d two hop received:%d r-received:%d\n", transactionRecords[transactionCounter].oneHopReportedPacketsForwardedTo, reportRespNoForwardMsgIn->responder, transactionRecords[transactionCounter].twoHopReportedPacketsReceived, reportRespNoForwardMsgIn->packetsReceived);
				#endif
              }
              break;
            }
