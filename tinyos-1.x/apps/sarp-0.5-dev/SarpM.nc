...
#define DATA_PACKET_INTERVAL                2000
...
#define MAX_TRANSACTION_CNT						150
#define DYNAMIC_FORGETTING_TIME				DATA_PACKET_INTERVAL * 3
...
//	uint32_t SWINDOW_TIME = (uint32_t)1000*60*60;
	...
	task void startApp()
	{
		...
		call Leds.greenOff();
		call Leds.yellowOff();
//		dbg(DBG_USR3, "Before initBehaviors::::myNodeID:%d\r\n", myAddr);
				...
				if (dataMsgIn->dst_node_id == myAddr)
				{
					...
					if (transactionFound == 0){
						...
						if(nGoodBehaviors <= 0 && nBadBehaviors == 0){
							nGoodBehaviors = myGoodBehaviors + 1;
							//nBadBehaviors = myBadBehaviors;
							nBadBehaviors = 0;						// For Testing a Temporary Error
						}
...
//				dbg(DBG_USR3, "specificReportReqMsgIn->transaction_id : %d\r\n", specificReportReqMsgIn->transaction_id);
				...
				thisTransaction = call TransactionTable.getThisTransactionPtr(transactionRecords, specificReportReqMsgIn->transaction_id);
  				#ifdef SARP04
				if(thisTransaction == NULL){	dbg(DBG_USR3, "%d : thisTransaction == NULL at %d\r\n", specificReportReqMsgIn->transaction_id, __LINE__);return;	}
				#endif
				atomic
				{
	  				#ifdef DEBUG
...
//		dbg(DBG_USR3, "addr: %d type: %d group:%d length:%d Data Length: %d\r\n", n->addr, n->type, n->group, n->length, TOS_DATA_LENGTH);
	...
	event result_t Timer2.fired()
	{
		...
		if (call TrafficGenerator.isTrafficGenerator(myAddr) == 1)
			call Timer6.start(TIMER_ONE_SHOT, DATA_PACKET_INTERVAL);
//		#ifdef PREDICTABILITY
		call Timer10.start(TIMER_REPEAT, DYNAMIC_FORGETTING_TIME);                     // starts dynamic forgetting
//		#else
			...
			if (checkThisTransaction->processed == 0)
			{
				...
				if ( (checkThisTransaction->oneHopReported > 0) && ( (checkThisTransaction->twoHopReported > 0) || (checkThisTransaction->packetsForwardedTo == 0)) )
				{
					if (checkThisTransaction->processed == 0) {
						...
						#ifdef ONOFFATTACK
						if(nGoodBehaviors >= 0)
							call TransactionTable.processThisTransaction(checkThisTransaction, neighbors, numNeighbors, myAddr, TRUE, cTime);
						else
							call TransactionTable.processThisTransaction(checkThisTransaction, neighbors, numNeighbors, myAddr, FALSE, cTime);
						#else
