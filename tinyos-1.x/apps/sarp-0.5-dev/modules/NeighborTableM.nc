...
implementation {
	 ...
	 */
	command result_t NeighborTable.processNeighborFromDiscMsg(Neighbor_Rec_withTrust_t neighbors[], Neighbor_Rec_withTrust_t *myOneHopNeighbors[], uint16_t *numNeighbors, uint16_t *numOneHopNeighbors, uint16_t nodeId_l, uint8_t level_l, uint16_t parent_l, uint16_t *isOneHop, uint32_t timeCounter)
	{
		atomic
		{
      #ifdef DEBUG
			...
			if (neighborFound == 0)
			{
				...
				memset(&neighbors[*numNeighbors].trust.GPB, 0, sizeof(GPB_t));
				memset(&neighbors[*numNeighbors].trust.BPB, 0, sizeof(BPB_t));
				neighbors[*numNeighbors].trust.GPB.Good_SWindow[0].Good = 1;
				neighbors[*numNeighbors].trust.GPB.Good_SWindow[0].Bad = 0;
				neighbors[*numNeighbors].trust.GPB.pointer = 0;
				neighbors[*numNeighbors].trust.BPB.Bad_SWindow[0].Good = 1;
				neighbors[*numNeighbors].trust.BPB.Bad_SWindow[0].Bad = 0;
				neighbors[*numNeighbors].trust.BPB.pointer = 0;
				#ifdef __REAL__MOTE__
				neighbors[*numNeighbors].trust.GPB.Good_SWindow[0].time = call LocalTime.read();
				neighbors[*numNeighbors].trust.BPB.Bad_SWindow[0].time = call LocalTime.read();
				#else
				neighbors[*numNeighbors].trust.GPB.Good_SWindow[0].time = tos_state.tos_time/4000;
				neighbors[*numNeighbors].trust.BPB.Bad_SWindow[0].time = tos_state.tos_time/4000;
				#endif
				if(TOS_LOCAL_ADDRESS == 7 && nodeId_l == 4){
					neighbors[*numNeighbors].trust.GPB.Good_SWindow[0].Good = 0;
					neighbors[*numNeighbors].trust.GPB.Good_SWindow[0].Bad = 1;
					neighbors[*numNeighbors].trust.GPB.pointer = 0;
					neighbors[*numNeighbors].trust.BPB.Bad_SWindow[0].Good = 0;
					neighbors[*numNeighbors].trust.BPB.Bad_SWindow[0].Bad = 1;
					neighbors[*numNeighbors].trust.BPB.pointer = 0;
					neighbors[*numNeighbors].trust.trustPR = 0.333;
				}
				neighbors[*numNeighbors].trust.overallTrust = call Trust.calculateOverallTrust(neighbors[*numNeighbors].trust.trustFP, neighbors[*numNeighbors].trust.trustLP, neighbors[*numNeighbors].trust.trustPR);
#else
				...
				neighbors[*numNeighbors].trust.GLB = TRUST_GLB_START;
				neighbors[*numNeighbors].trust.BLB = TRUST_BLB_START;
				neighbors[*numNeighbors].trust.overallTrust = 0.67;
				#endif
#endif
	...
	command uint16_t NeighborTable.getNewTransactionParent(Neighbor_Rec_withTrust_t *myOneHopNeighbors[], uint16_t numOneHopNeighbors, uint8_t myLevel_l, Neighbor_Rec_withTrust_t *myPeers[], uint16_t numPeers, Neighbor_Rec_withTrust_t *myChildren[], uint16_t numChildren, uint16_t dontPickMe, uint16_t dontPickMeEither, uint16_t reallyDontPickMe, uint16_t whoIAm)
	{
		...
		//dbg(DBG_USR3, "Parent num one-hops: %d\r\n", numOneHopNeighbors);
    #ifdef DEBUG
    ...
    #endif
//	 	dbg(DBG_USR3, "%d:%d chose %d:%d as its transaction parent, with a trust value of %f\r\n", TOS_LOCAL_ADDRESS, myLevel_l, myOneHopNeighbors[chosenParentTrustIndex]->node_id, myOneHopNeighbors[chosenParentTrustIndex]->level, myOneHopNeighbors[chosenParentTrustIndex]->trust.overallTrust);
